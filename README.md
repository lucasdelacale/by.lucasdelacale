...
O projeto não possui backend, banco de dados, autenticação ou painel administrativo. Publicar conteúdo significa adicionar arquivos Markdown e imagens ao projeto.
...
Para exibir uma obra nas páginas específicas, use `type: print` ou `type: canvas` no frontmatter. As obras com `type: fotografia` continuam no Arquivo geral. O nome do arquivo ou o nome da imagem não define a categoria: a classificação vem do campo `type`.
